...
while true; do
    ...
    if [[ "$archval" == "x86" ]] || [[ "$archval" == "X86" ]]; then                                             # installs x86_toolchain.sh and qemu
        if ! [ -x "$(command -v x86_toolchain.sh)" ]; then                                                      # checks if the toolchain exists
            sudo wget -P /usr/bin/ https://raw.githubusercontent.com/LubosKuzma/AssemblyToolchains/main/scripts/x86_toolchain.sh
        ...
        if ! [ -x "$(command -v arm_toolchain.sh)" ]; then                                                      # checks if the toolchain doesn't exist
            ...
            wget -P /usr/bin https://raw.githubusercontent.com/LubosKuzma/AssemblyToolchains/main/scripts/arm_toolchain.sh
